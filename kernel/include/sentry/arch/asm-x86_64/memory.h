...
static inline uint8_t mpu_get_id_from_ressource(const layout_resource_t ressource __attribute__((unused)))
{
    /* sample function: fixing region id to 3 here */
    return 3;
}

static inline kstatus_t mpu_forge_unmapped_ressource(uint8_t id __attribute__((unused)), layout_resource_t* ressource)
{
    if (ressource == NULL) {
